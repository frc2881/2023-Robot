// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands.arm;

import edu.wpi.first.wpilibj2.command.SequentialCommandGroup;
import frc.robot.commands.intake.ExtendIntakeArm;
import frc.robot.commands.suction.DisableSuction;
import frc.robot.commands.suction.EnableSuction;
import frc.robot.subsystems.Arm;
import frc.robot.subsystems.Intake;
import frc.robot.subsystems.Suction;

// NOTE:  Consider using this command inline, rather than writing a subclass.  For more
// information, see:
// https://docs.wpilib.org/en/stable/docs/software/commandbased/convenience-features.html
public class ScoreHigh extends SequentialCommandGroup {
  
  public ScoreHigh(Arm arm, Intake intake, Suction suction, Double speed) {
    
    if(intake.isOut = false) {
      addCommands(new ExtendIntakeArm(intake));
    }

    addCommands(
      new TiltArmToHeight(arm, intake, speed, 2.0),
      new ExtendArmToLength(arm, speed, 3.0),
      new EnableSuction(suction),
      new TiltArmToHeight(arm, intake, speed, 7.0),
      new ExtendArmToLength(arm, speed, 7.0),
      new DisableSuction(suction));
  }

}
